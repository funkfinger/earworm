...
# Script to run all tests for the De Worm application
...
echo -e "${GREEN}=== De Worm Testing Suite ===${NC}"
...
echo "Waiting for Playwright tests to complete (max 90s)..."
npx playwright test
PLAYWRIGHT_EXIT_CODE=$?

if [ $PLAYWRIGHT_EXIT_CODE -ne 0 ]; then
  echo -e "${RED}✗ E2E tests failed or timed out (exit code: $PLAYWRIGHT_EXIT_CODE)${NC}"
