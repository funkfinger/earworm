...
# Script to run quick Safari-only E2E tests for the De Worm application
...
echo -e "${GREEN}=== De Worm Quick Safari Testing Suite ===${NC}"
echo -e "${GREEN}Running quick Safari E2E tests...${NC}"
echo ""
...
npx playwright test --config=playwright.safari-quick.config.ts
PLAYWRIGHT_EXIT_CODE=$?

if [ $PLAYWRIGHT_EXIT_CODE -ne 0 ]; then
  echo -e "${RED}✗ Quick Safari E2E tests failed or timed out (exit code: $PLAYWRIGHT_EXIT_CODE)${NC}"
  ALL_TESTS_PASSED=false
else
  echo -e "${GREEN}✓ Quick Safari E2E tests passed${NC}"
fi
